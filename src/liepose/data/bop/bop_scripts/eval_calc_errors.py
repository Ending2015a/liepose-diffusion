...
for result_filename in p["result_filenames"]:
  ...
  # Load object models.
  models = {}
  if p["error_type"] in ["ad", "add", "adi", "mssd", "mspd", "proj", "projS"]:
  ...
  ests_org = {}
  for est in ests:
    ests_org.setdefault(est["scene_id"], {}).setdefault(est["im_id"], {}).setdefault(
      est["obj_id"], []
    ).append(est)
  ...
  for scene_id, scene_targets in targets_org.items():
    ...
    for im_ind, (im_id, im_targets) in enumerate(scene_targets.items()):
      ...
      for obj_id, target in im_targets.items():
        ...
        if not p["skip_missing"] and obj_count < n_top_curr:
          raise ValueError(
            f"Not enough estimates for scene: {scene_id}, im: {im_id}, obj: {obj_id} "
            f"(provided: {obj_count}, expected: {n_top_curr})"
          )
        ...
        for est_id, est in obj_ests_sorted:
          ...
          for gt_id, gt in enumerate(scene_gt[im_id]):
            ...
            if p["error_type"] == "vsd":
              ...
              e = [
                pose_error.mspd(
                  R_e, t_e, R_g, t_g, K, models[obj_id]["pts"], models_sym[obj_id]
                )
              ]
            ...
            elif p["error_type"] == "projS":
              w = [
                pose_error.arp_2d_sym(
                  R_e, t_e, R_g, t_g, K, models[obj_id]["pts"], models_sym[obj_id]
                )
              ]
              ...
              e = [
                pose_error.re_sym(R_e, R_g, syms=models_sym[obj_id]),
                pose_error.te_sym(t_e, t_g, R_gt=R_g, syms=models_sym[obj_id]) / 10,
              ]

            elif p["error_type"] == "reS":
              e = [pose_error.re_sym(R_e, R_g, syms=models_sym[obj_id])]
            ...
            elif p["error_type"] == "tex":
              t_e_copy = copy.deepcopy(t_e)
              t_g_copy = copy.deepcopy(t_g)
              t_e_copy[1] = 0.0
              t_e_copy[2] = 0.0
              t_g_copy[1] = 0.0
              t_g_copy[2] = 0.0
              e = [pose_error.te(t_e_copy, t_g_copy)]

            elif p["error_type"] == "tey":
              t_e_copy = copy.deepcopy(t_e)
              t_g_copy = copy.deepcopy(t_g)
              t_e_copy[0] = 0.0
              t_e_copy[2] = 0.0
              ...
              t_g_copy[2] = 0.0
              e = [
                pose_error.te_sym(t_e_copy, t_g_copy, R_gt=R_g, syms=models_sym[obj_id])
                / 10
              ]

            elif p["error_type"] == "teSy":
              t_e_copy = copy.deepcopy(t_e)
              t_g_copy = copy.deepcopy(t_g)
              t_e_copy[0] = 0.0
              t_e_copy[2] = 0.0
    ...
    def save_errors(
      _error_sign, _scene_errs, scene_id=scene_id, result_name=result_name
    ):
  ...
  misc.log(f"Calculation of errors for {ests_counter} estimates took {time_total}s.")
