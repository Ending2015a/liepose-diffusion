...
from liepose.dist import NormalSO3
from liepose.exp.symsol.testbed import Testbed
...
class TestbedSO3(Testbed):
  Lie = SO3
  LieDist = NormalSO3
  lie_metrics = so3_metrics
...
def run_experiment(a: Options, exp_path: str):
  ...
  exp = TestbedSO3(a, exp_path, setup=True)
...
def main():
  a = get_args(exp_name="symsol-score-flat", desc="Symsol experiment")
  ...
  for exploration in tqdm(explorations, desc="Explorations"):
    exp_name = exploration.to_string(includes=["timesteps", "repr_type", "seed"])
